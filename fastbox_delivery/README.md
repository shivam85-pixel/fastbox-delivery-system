...
1. **Random Delivery Delays** — Seeded random delays (0–30 min per leg) added per agent; printed to console during simulation.
2. **ASCII Route Visualisation** — Each agent's delivery path rendered as a 2D ASCII grid map (printed to console).
3. **Export Top Performer to CSV** — Best agent's stats saved to `top_performer.csv`.
